...
<html lang="en">
<head>
	<title> Events to Complete </title>
	<meta charset="utf-8">
					...
					<div class="card-block">
						<table id="table" class="table table-bordered" style="background:#fff;">					
							...
							<tbody>						
								...
								$query = "SELECT * FROM  events WHERE LeadVounteer = '$user' and completed='1'";
								$result = executeSelectQuery($query);
								if ($result->num_rows > 0){
									while($req_row = $result->fetch_assoc()) {										
								?>
								<tr>
									<td><a href="complete.php?user=<?php echo $user ?>&id=<?php echo $req_row['intId']; ?>"><span class='label label-custom label-pill label-success'>complete</span></a></td>
									<td><?php echo $req_row['intId']; ?></td>
